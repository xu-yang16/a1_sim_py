...
class VelocityControl:
    ...
    def __init__(self, node):
        self._node = node
        self._num_steps = self._node.declare_parameter('turbo/steps', 1).value

        forward_min = self._node.declare_parameter(
            'turbo/linear_forward_min', 1.0).value
        forward_max = self._node.declare_parameter(
            'turbo/linear_forward_max', 1.0).value
        ...
        lateral_min = self._node.declare_parameter(
            'turbo/linear_lateral_min', 1.0).value
        lateral_max = self._node.declare_parameter(
            'turbo/linear_lateral_max', 1.0).value
        self._lateral = Velocity(lateral_min, lateral_max, self._num_steps)

        angular_min = self._node.declare_parameter('turbo/angular_min', 1.0).value
        angular_max = self._node.declare_parameter('turbo/angular_max', 1.0).value
        ...
        init_step = self._node.declare_parameter(
            'turbo/init_step', default_init_step).value
...
class JoystickRelay(Node):
    ...
    def __init__(self, name):
        ...
        _msg = Bool()
        _msg.data = self._current_priority.value
        self._pub_priority.publish(_msg)
...
def main(args=None):
    ...
    node = JoystickRelay("node_joystick_relay")
